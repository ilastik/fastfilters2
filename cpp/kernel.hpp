...
// Compute the size of the kernel for a given scale (standard deviation, a.k.a. sigma)
// and the kernel order. If truncate is positive, the kernel is truncated at `truncate *
// scale` standard deviations; otherwise, use an internal order-dependent default.
...
// Fill the kernel with the values of a Gaussian kernel or Gaussian derivative of the
// given scale and order.
inline void gaussian_kernel(float *kernel, size_t ksize, double scale, int order) {
    ...
    // Compute the initial kernel values; even though the normalization factor is used
    // in the formula, it is necessary to numerically normalize the coefficients later.
    ...
    if (order == 2) {
        // Remove the DC component (mean value) from the 2-nd order derivative that can
        // be non-zero due to the numerical inaccuracies.
        double sum = kernel[0];
    ...
    // Numerically normalize the corresponding moment of the kernel to 1.
